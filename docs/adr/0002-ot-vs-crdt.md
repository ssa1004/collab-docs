...
- **CRDT(충돌없는 복제 자료형)** — 각 글자에 전역 유일 id를 붙여, 누가 먼저 합치든 순서에 상관없이 같은 결과가 나오게 설계한 자료구조. 심판(서버) 없이도 알아서 합쳐진다.
- **교환법칙(commutativity)** — 'A 다음 B'든 'B 다음 A'든 합친 결과가 같다는 성질. CRDT가 순서 걱정 없이 병합되는 비결.
- **rebase(리베이스)** — 내가 옛 버전을 보고 만든 편집을, 그새 남이 먼저 넣은 편집들 위로 위치를 다시 얹어 끼워 맞추는 것.
- **tombstone(묘비 표식)** — 글자를 지워도 진짜로 없애지 않고 '삭제됨' 표시만 남기는 흔적. CRDT는 이게 계속 쌓여 메모리를 잡아먹는다.
- **TP1 / TP2(transform 속성)** — TP1은 두 편집을 어느 순서로 합쳐도 최종 글자가 같아야 한다는 규칙(수렴). TP2는 거기서 더 나아가 분산·재정렬 환경에서 변환이 어떤 식으로 엮여도 일관돼야 한다는, OT에서 가장 풀기 어려운 규칙. 서버가 순서를 정해주면 TP2는 피하고 TP1만 지키면 된다.
- **active-active(다중 권위)** — 여러 지역의 서버가 모두 '주인' 역할을 하며 각자 쓰기를 받는 구성. 중앙 한 곳이 순서를 정하는 모델이 깨져, 병합이 훨씬 어려워진다.
- **GC(가비지 컬렉션)** — 더 안 쓰는 메모리를 JVM이 알아서 치우는 청소 작업. tombstone이 무한정 쌓이면 이 청소가 무거워진다.

## 참고

- Ellis, Gibbs — [Concurrency control in groupware systems](https://dl.acm.org/doi/10.1145/67544.66963) (1989) — OT 의 출발점
- Sun, Ellis — [Operational transformation in real-time group editors: issues, algorithms, and achievements](https://dl.acm.org/doi/10.1145/289444.289469) (1998) — TP1/TP2 정식화
- Shapiro 외 — [A comprehensive study of Convergent and Commutative Replicated Data Types](https://hal.inria.fr/inria-00555588) (2011) — CRDT 기반
- [Yjs](https://github.com/yjs/yjs) / [Automerge](https://github.com/automerge/automerge) — 보류한 CRDT 진영의 대표 구현
