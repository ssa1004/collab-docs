...
- **fan-out(팬아웃)** — 한 사람의 편집·커서를 같은 문서를 보는 모든 접속자에게 한꺼번에 쫙 뿌리는 것. 선풍기 날개처럼 하나가 여러 갈래로.
- **presence(실시간 존재 표시)** — 같은 문서를 보는 사람들의 커서 위치·선택 영역·'지금 타이핑 중'을 서로에게 실시간으로 보여주는 것(구글 docs의 색깔 커서).
- **pub/sub(발행-구독)** — 보내는 쪽은 '문서 채널에 발행'만 하고, 받을 사람은 그 채널을 '구독'해 두면 알아서 받는 우체통 방식. 누가 듣는지 몰라도 돼 사람이 늘어도 코드가 안 변한다.
- **WebSocket / STOMP push** — 서버가 먼저 데이터를 밀어주는 실시간 연결. 보통 웹은 물어봐야 답하지만, 이건 통화처럼 양쪽이 계속 연결돼 서버가 알아서 보낸다.
- **at-most-once(최대 한 번 — 유실 가능)** — 메시지가 많아야 한 번 도착. 중복은 없지만 운 나쁘면 못 받을 수도 있는 약한 보장. 놓쳐도 진짜 데이터는 DB에 있으니 재접속해 따라잡으면 된다.
- **edge 게이트웨이 / 멀티플렉싱** — 수많은 WebSocket 연결을 맨 앞에서 받아 인증하고 중계만 하는 얇은 문지기 서버. 멀티플렉싱은 한 통로에 여러 연결·메시지를 겹쳐 실어 효율적으로 다루는 것. 편집 충돌 계산(OT)은 절대 안 한다.
- **best-effort(최선 노력)** — '되도록 보내되 못 가도 책임 안 짐'. presence처럼 유실돼도 다음 신호로 곧 자가 치유되는 데이터에 쓰는 느슨한 보장.
